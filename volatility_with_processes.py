...
class Volatilly(Process):
    ...
    def run(self):
        ...
        self.conn.send([name_ticket, volatility])
        self.conn.close()
...
def funk():
    ...
    for dirpath, dirnames, filenames in os.walk(path_normalized):
        for file in filenames:
            ...
            vol1 = Volatilly(data=full_file_path, conn=child_conn)
            lst1.append(vol1)
    ...
    for i in lst1:
        i.start()
    for conn in pipes:
        name, volat = conn.recv()
        lst.append([name, volat])
